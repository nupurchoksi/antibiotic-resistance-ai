...
def plot_zone_distributions(df2):
    ...
    for i, ab in enumerate(CLSI_BREAKPOINTS):
        bp = CLSI_BREAKPOINTS[ab]
        ax = axes[i]
        ax.hist(df2[ab].dropna(), bins=15, color="#3B8BD4", alpha=0.75, edgecolor="white")
...
def plot_mdr_analysis(df):
    ...
    species_mdr = df.groupby("Species_clean")["is_MDR"].mean().loc[top_species] * 100
